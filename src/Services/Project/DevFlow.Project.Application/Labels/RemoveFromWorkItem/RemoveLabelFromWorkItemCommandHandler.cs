...
internal sealed class RemoveLabelFromWorkItemCommandHandler
    : IRequestHandler<
        RemoveLabelFromWorkItemCommand,
        Result<RemoveLabelFromWorkItemResponse>>
{
    private readonly IWorkItemRepository _workItemRepository;
    private readonly ILabelRepository _labelRepository;
    private readonly IUnitOfWork _unitOfWork;

    public RemoveLabelFromWorkItemCommandHandler(
    ...
    public async Task<Result<RemoveLabelFromWorkItemResponse>> Handle(
        RemoveLabelFromWorkItemCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (workItem is null)
        {
            return Result.Failure<RemoveLabelFromWorkItemResponse>(
        ...
        workItem.RemoveLabel(label.Id.Value);

        await _unitOfWork.SaveChangesAsync(
            cancellationToken);

        return Result.Success(
            new RemoveLabelFromWorkItemResponse(
                workItem.Id.Value,
                label.Id.Value),
